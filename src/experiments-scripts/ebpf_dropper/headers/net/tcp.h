...
#ifdef CONFIG_MPTCP
extern int sysctl_mptcp_aggressive_rto __read_mostly;
extern int sysctl_mptcp_aggressive_dupack __read_mostly;
#endif
...
static inline u32 __tcp_set_rto(const struct tcp_sock *tp)
{
	u32 delay;
#ifdef CONFIG_MPTCP
	if (tp -> meta_sk && sysctl_mptcp_aggressive_rto && tp->rx_opt.tstamp_extended){ 
		// TODO: handle different RTT precisions
		delay = tp -> owd_out.delay*1000 + (tcp_sk(tp -> meta_sk) -> mptcp_min_path_owd_in*1000);
		printk("DEBUG_AGGRESSIVE set rto = %u, forward owd = %u, min backward owd: %u, var = %u, srtt = %u, rto otherwise = %u", (delay >> 3) + tp->rttvar_us, tp -> owd_out.delay >> 3, tcp_sk(tp -> meta_sk) -> mptcp_min_path_owd_in >> 3, tp->rttvar_us, tp->srtt_us >> 3, (tp->srtt_us >> 3) + tp->rttvar_us);
		printk("DEBUG_AGGRESSIVE set rto: saddr: %x, daddr: %x", ((struct sock *) tp) -> sk_rcv_saddr, ((struct sock *) tp) -> sk_daddr);
	}
	else
		delay = tp->srtt_us;
#else
	delay = tp->srtt_us;
#endif
	printk("DEBUG_AGGRESSIVE set rto = %u (RTT = %u)", (delay >> 3) + tp->rttvar_us, tp->srtt_us >> 3);
	//TODO: decide for a correct variance in the aggressive_rto case
	return usecs_to_jiffies((delay >> 3) + tp->rttvar_us);
}
