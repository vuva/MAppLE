...
name1 = args.namefirsttest.split(",") if args.namefirsttest is not None else [None]
name2 = args.namesecondtest.split(",") if args.namesecondtest is not None else [None]
name3 = args.namethirdtest.split(",") if args.namethirdtest is not None else [None]
...
for i, (filename, filename2, filename3, METHOD_1, METHOD_2, METHOD_3) in enumerate(zip(files, files2, files3, name1, name2, name3)):
    ...
    def plot_cdf(x_axes, y_axis, transform, color=None, label=None, linestyle=None):
        ...
        if idx == 0:
            ...
            plt.ylabel(" ")
        plt.ylabel("%s" % transform_unit[transform] if args.xlabel is None else args.xlabel)
        if transforms_output[transform] > 1 or color is None or label is None:
            ...
            print((label))
            plt.xlim(-1, 5)
            return (plt.boxplot(x_axes, notch=False, labels=label, widths=0.5, positions=(1, 2, 3))["boxes"]) #, label=transforms_labels[transform][i] if l is None else l)
        else:
    ...
    if filename[-5:] == ".json":
        ...
        def plot(filesize, color=None, label=None, linestyle=None):
            if c2 is None:
                ...
                if join:
                    ...
                    for i, elem in enumerate(res1):
                        ...
                        #     break
                        if c3 is not None:
                            res.append((elem[0], tmp_res[i][0] if i < len(tmp_res) else None, res3[i][0] if i < len(res3) else None))
                        else:
            ...
            to_plot = [list(sorted([r[i] for r in transformed_result if r[i] is not None])) for i in range(transforms_output[transform])]
            ...
            if plot_method == "CDF":
                plot_cdf(to_plot, y_axis, transform, color, legends, linestyle)
