	...
	Context("for gQUIC", func() {
		Context("parsing", func() {
			It("sets all values", func() {
				...
				Expect(params.MaxPathID).To(Equal(protocol.PathID(3)))
			})
			...
			It("errors when given an invalid MPID value", func() {
				values := map[Tag][]byte{TagMPID: {2, 0, 0}} // 1 byte too short
				_, err := readHelloMap(values)
				Expect(err).To(MatchError(errMalformedTag))
			})
		})
		...
		Context("writing", func() {
			It("returns all necessary parameters ", func() {
				params := &TransportParameters{
					...
					MaxStreams:                  0x1337,
					MaxPathID:                   0x3,
					FECScheme:                   protocol.ReedSolomonFECScheme,
				}
				entryMap := params.getHelloMap()
				Expect(entryMap).To(HaveLen(6))
				...
				Expect(entryMap).To(HaveKeyWithValue(TagMPID, []byte{0x3, 0, 0, 0}))
				Expect(entryMap).To(HaveKeyWithValue(TagFSOP, []byte{0x1, 0, 0, 0}))
			})
	...
	Context("for TLS", func() {
		...
		Context("parsing", func() {
			...
			It("reads parameters", func() {
				...
				Expect(params.OmitConnectionID).To(BeFalse())
				Expect(params.MaxPathID).To(Equal(protocol.PathID(0x3)))
			})
			...
			It("rejects the parameters if max_path_id has the wrong length", func() {
				parameters[maxPathIDParameterID] = []byte{0, 0, 0} // should be 1 byte
				_, err := readTransportParamters(paramsMapToList(parameters))
				Expect(err).To(MatchError("wrong length for max_path_id: 3 (expected 1)"))
			})
		...
		Context("writing", func() {
			...
			It("creates the parameters list", func() {
				...
				Expect(values).To(HaveKeyWithValue(maxPathIDParameterID, []byte{0x3}))
				Expect(values).To(HaveKeyWithValue(fecSchemeParameterID, []byte{protocol.ReedSolomonFECScheme}))
			})
