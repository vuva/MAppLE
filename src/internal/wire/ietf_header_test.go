...
var _ = Describe("IETF draft Header", func() {
	...
	Context("writing", func() {
		...
		Context("short header", func() {
			...
			It("writes a header with connection ID and path ID", func() {
				err := (&Header{
					ConnectionID:    0xdeadbeefcafe1337,
					PathID:          0x4,
					PacketNumberLen: protocol.PacketNumberLen1,
	...
	Context("logging", func() {
		...
		It("logs Short Headers containing a connection ID", func() {
			...
			Expect(string(buf.Bytes())).To(ContainSubstring("Short Header{ConnectionID: 0xdeadbeef, PathID: 0x0, PacketNumber: 0x1337, PacketNumberLen: 4, KeyPhase: 1}"))
		})
		...
		It("logs Short Headers with omitted connection ID", func() {
			...
			Expect(string(buf.Bytes())).To(ContainSubstring("Short Header{ConnectionID: (omitted), PathID: 0x0, PacketNumber: 0x12, PacketNumberLen: 1, KeyPhase: 0}"))
		})

		It("logs Short Headers containing a path ID", func() {
			(&Header{
				KeyPhase:        1,
				PacketNumber:    0x1337,
				PacketNumberLen: 4,
			...
			Expect(string(buf.Bytes())).To(ContainSubstring("Short Header{ConnectionID: 0xdeadbeef, PathID: 0x7, PacketNumber: 0x1337, PacketNumberLen: 4, KeyPhase: 1}"))
		})
	})
})
