...
var _ = Describe("Public Header", func() {
	Context("when parsing", func() {
		It("accepts a sample client header", func() {
			...
			b := bytes.NewReader(append(append([]byte{0x09, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6}, ver...), 0x01, 0x0))
			...
			Expect(hdr.PathID).To(Equal(protocol.PathID(0)))
			Expect(b.Len()).To(BeZero())
		})

		It("does not accept an omittedd connection ID as a server", func() {
			b := bytes.NewReader([]byte{0x00, 0x01, 0x0})
		...
		It("rejects 0 as a connection ID", func() {
			b := bytes.NewReader([]byte{0x09, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x51, 0x30, 0x33, 0x30, 0x01, 0x0})
			_, err := parsePublicHeader(b, protocol.PerspectiveClient)
			Expect(err).To(MatchError(errInvalidConnectionID))
		})

		It("reads a PublicReset packet", func() {
			b := bytes.NewReader([]byte{0xa, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x0})
			hdr, err := parsePublicHeader(b, protocol.PerspectiveServer)
			Expect(err).ToNot(HaveOccurred())
		...
		It("parses a public reset packet", func() {
			b := bytes.NewReader([]byte{0xa, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0})
			hdr, err := parsePublicHeader(b, protocol.PerspectiveServer)
			Expect(err).ToNot(HaveOccurred())
			Expect(hdr.ResetFlag).To(BeTrue())
		...
		It("reads a diversification nonce sent by the server", func() {
			...
			b := bytes.NewReader(append(append([]byte{0x0c, 0xf6, 0x19, 0x86, 0x66, 0x9b, 0x9f, 0xfa, 0x4c}, divNonce...), 0x37, 0x0))
			hdr, err := parsePublicHeader(b, protocol.PerspectiveServer)
			Expect(err).ToNot(HaveOccurred())
		...
		Context("Packet Number lengths", func() {
			It("accepts 1-byte packet numbers", func() {
				b := bytes.NewReader([]byte{0x08, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xde, 0x0})
				hdr, err := parsePublicHeader(b, protocol.PerspectiveClient)
				Expect(err).ToNot(HaveOccurred())
			...
			It("accepts 2-byte packet numbers", func() {
				b := bytes.NewReader([]byte{0x18, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xde, 0xca, 0x0})
				hdr, err := parsePublicHeader(b, protocol.PerspectiveClient)
				Expect(err).ToNot(HaveOccurred())
			...
			It("accepts 4-byte packet numbers", func() {
				b := bytes.NewReader([]byte{0x28, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xad, 0xfb, 0xca, 0xde, 0x0})
				hdr, err := parsePublicHeader(b, protocol.PerspectiveClient)
				Expect(err).ToNot(HaveOccurred())
			...
			It("accepts 6-byte packet numbers", func() {
				b := bytes.NewReader([]byte{0x38, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0x23, 0x42, 0xad, 0xfb, 0xca, 0xde, 0x0})
				hdr, err := parsePublicHeader(b, protocol.PerspectiveClient)
				Expect(err).ToNot(HaveOccurred())
			...
			It("accepts 2-byte packet numbers and a Path ID", func() {
				b := bytes.NewReader([]byte{0x18, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xde, 0xca, 0x4})
				hdr, err := parsePublicHeader(b, protocol.PerspectiveClient)
				Expect(err).ToNot(HaveOccurred())
				Expect(hdr.PathID).To(Equal(protocol.PathID(4)))
				Expect(hdr.PacketNumber).To(Equal(protocol.PacketNumber(0xdeca)))
				Expect(hdr.PacketNumberLen).To(Equal(protocol.PacketNumberLen2))
				Expect(b.Len()).To(BeZero())
			})
	...
	Context("when writing", func() {
		It("writes a sample header as a server", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(b.Bytes()).To(Equal([]byte{0x38, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0, 0, 0, 0, 0, 2, 0}))
		})
		...
		It("writes a sample header as a client", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(b.Bytes()).To(Equal([]byte{0x38, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0x0, 0x0, 0x0, 0x0, 0x13, 0x37, 0x0}))
		})
		...
		It("omits the connection ID", func() {
			...
			err := hdr.writePublicHeader(b, protocol.PerspectiveServer, protocol.VersionWhatever)
			Expect(err).ToNot(HaveOccurred())
			Expect(b.Bytes()).To(Equal([]byte{0x0, 0x1, 0x0}))
		})
		...
		Context("Version Negotiation packets", func() {
			...
			It("sets the Version Flag for packets sent as a client, and adds a packet number", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				// must be the first assertion
				Expect(b.Len()).To(Equal(1 + 8 + 4 + 1 + 1)) // 1 FlagByte + 8 ConnectionID + 4 version number + 1 PacketNumber + 1 PathID
				firstByte, _ := b.ReadByte()
				Expect(firstByte & 0x01).To(Equal(uint8(1)))
				Expect(firstByte & 0x30).To(Equal(uint8(0x0)))
				Expect(string(b.Bytes()[8:12])).To(Equal("Q039"))
				Expect(b.Bytes()[12:13]).To(Equal([]byte{0x42}))
				Expect(b.Bytes()[13:14]).To(Equal([]byte{0x0}))
			})
		})
		...
		Context("getting the length", func() {
			...
			It("gets the length of a packet with longest packet number length and connectionID", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 8 + 6 + 1))) // 1 byte public flag, 8 bytes connectionID, and packet number + 1 for PathID
			})
			...
			It("gets the lengths of a packet sent by the client with the VersionFlag set", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 4 + 6 + 1))) // 1 byte public flag, 4 version number, and packet number and Path ID
			})
			...
			It("gets the length of a packet with longest packet number length and omitted connectionID", func() {
				...
				length, err := hdr.getPublicHeaderLength(protocol.PerspectiveServer)
				Expect(err).ToNot(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 6 + 1))) // 1 byte public flag, and packet number and PAthID
			})
			...
			It("gets the length of a packet 2 byte packet number length ", func() {
				...
				length, err := hdr.getPublicHeaderLength(protocol.PerspectiveServer)
				Expect(err).ToNot(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 8 + 2 + 1))) // 1 byte public flag, 8 byte connectionID, and packet number and PathID
			})

			It("gets the length of a packet 2 byte packet number length with a path ID", func() {
				hdr := Header{
					ConnectionID:    0x4cfa9f9b668619f6,
					PathID:          0x4,
					PacketNumber:    0xDECAFBAD,
				...
				length, err := hdr.getPublicHeaderLength(protocol.PerspectiveServer)
				Expect(err).ToNot(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 8 + 1 + 2))) // 1 byte public flag, 8 byte connectionID, 1 byte pathID and packet number
			})
			...
			It("works with diversification nonce", func() {
				...
				Expect(length).To(Equal(protocol.ByteCount(1 + 8 + 3 + 1 + 1))) // 1 byte public flag, 8 byte connectionID, 3 byte DiversificationNonce, 1 byte PacketNumber and 1 byte PathID
			})
			...
			It("gets the length of a PublicReset", func() {
				...
				length, err := hdr.getPublicHeaderLength(protocol.PerspectiveServer)
				Expect(err).NotTo(HaveOccurred())
				Expect(length).To(Equal(protocol.ByteCount(1 + 8 + 1))) // 1 byte public flag, 8 byte connectionID and 1 byte Path ID
			})
		})
		...
		Context("packet number length", func() {
			...
			Context("in big endian", func() {
				...
				It("writes a header with a 1-byte packet number", func() {
					...
					Expect(err).ToNot(HaveOccurred())
					Expect(b.Bytes()).To(Equal([]byte{0x08, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xad, 0x0}))
				})
				...
				It("writes a header with a 2-byte packet number", func() {
					...
					err := hdr.writePublicHeader(b, protocol.PerspectiveServer, version)
					Expect(err).ToNot(HaveOccurred())
					Expect(b.Bytes()).To(Equal([]byte{0x18, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xfb, 0xad, 0x0}))
				})
				...
				It("writes a header with a 4-byte packet number", func() {
					...
					err := hdr.writePublicHeader(b, protocol.PerspectiveServer, version)
					Expect(err).ToNot(HaveOccurred())
					Expect(b.Bytes()).To(Equal([]byte{0x28, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xde, 0xca, 0xfb, 0xad, 0x0}))
				})
				...
				It("writes a header with a 6-byte packet number", func() {
					...
					err := hdr.writePublicHeader(b, protocol.PerspectiveServer, version)
					Expect(err).ToNot(HaveOccurred())
					Expect(b.Bytes()).To(Equal([]byte{0x38, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0x13, 0x37, 0xde, 0xca, 0xfb, 0xad, 0x0}))
				})

				It("writes a header with a 2-byte packet number and a Path ID", func() {
					b := &bytes.Buffer{}
					hdr := Header{
						ConnectionID:    0x4cfa9f9b668619f6,
						PathID:          0x7,
					...
					err := hdr.writePublicHeader(b, protocol.PerspectiveServer, version)
					Expect(err).ToNot(HaveOccurred())
					Expect(b.Bytes()).To(Equal([]byte{0x18, 0x4c, 0xfa, 0x9f, 0x9b, 0x66, 0x86, 0x19, 0xf6, 0xfb, 0xad, 0x7}))
				})
			})
		})
	})
	...
	Context("logging", func() {
		...
		It("logs a Public Header containing a connection ID", func() {
			...
			Expect(string(buf.Bytes())).To(ContainSubstring("Public Header{ConnectionID: 0xdecafbad, PathID: 0x0, PacketNumber: 0x1337, PacketNumberLen: 6, Version: gQUIC 39"))
		})
		...
		It("logs a Public Header containing a path ID", func() {
			(&Header{
				ConnectionID:    0xdecafbad,
				PathID:          4,
				PacketNumber:    0x1337,
			...
			Expect(string(buf.Bytes())).To(ContainSubstring("Public Header{ConnectionID: 0xdecafbad, PathID: 0x4, PacketNumber: 0x1337, PacketNumberLen: 6, Version: gQUIC 39"))
		})
